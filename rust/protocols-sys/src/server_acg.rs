...
pub trait ServerACG {
    ...
    fn preprocess(
        &mut self,
        linear_share: &Output<u64>,
        linear_mac_share: &Output<u64>,
        r_mac_share: &Input<u64>,
        mac_key_a: u64,
        mac_key_b: u64,
    );

    fn process(&mut self, client_share: Vec<c_char>) -> (Vec<c_char>, Vec<c_char>, Vec<c_char>);
}

impl<'a> SealServerACG<'a> {
    pub fn preprocess(
        &mut self,
        linear_share: &Output<u64>,
        linear_mac_share: &Output<u64>,
        ...
        match self {
            ...
            Self::FullyConnected(s) => s.preprocess(
                linear_share,
                linear_mac_share,
                r_mac_share,
                mac_key_a,
...
impl<'a> ServerACG for Conv2D<'a> {
    ...
    fn preprocess(
        ...
        let mut linear_mac_vec: Vec<_> = vec![std::ptr::null(); self.data.out_chans as usize];
        let mut r_mac_vec: Vec<_> = vec![std::ptr::null(); self.data.inp_chans as usize];
        ...
        for out_c in 0..self.data.out_chans as usize {
            ...
            linear_mac_vec[out_c] = linear_mac_share
                .slice(s![0, out_c, .., ..])
                .as_slice()
                .expect("Error converting server randomness")
                .as_ptr();
        }
        for inp_c in 0..self.data.inp_chans as usize {
            r_mac_vec[inp_c] = r_mac_share
                .slice(s![0, inp_c, .., ..])
                .as_slice()
                .expect("Error converting server randomness")
                .as_ptr();
        ...
        self.shares = Some(unsafe {
            server_conv_preprocess_shares(
                self.sfhe,
                &self.data,
                linear_vec.as_ptr(),
                linear_mac_vec.as_ptr(),
                r_mac_vec.as_ptr(),
                mac_key_a,
                mac_key_b,
            )
    ...
    fn process(
        ...
        let linear_mac_ct_vec = unsafe {
            std::slice::from_raw_parts(
                shares.linear_mac_ct.inner,
                shares.linear_mac_ct.size as usize,
            )
            .to_vec()
        };
...
impl<'a> ServerACG for FullyConnected<'a> {
    type Keys = &'a ServerFHE;
    ...
    fn preprocess(
        ...
        self.shares = Some(unsafe {
            server_fc_preprocess_shares(
                self.sfhe, &self.data, linear, linear_mac, r_mac, mac_key_a, mac_key_b,
            )
        });
    }
